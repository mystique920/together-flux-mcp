⚠️ **LibreChat-Exclusive Branch**

**This branch is designed exclusively for integration with LibreChat.**

- Images generated by this MCP server are saved locally in a user-specific directory (`/app/client/public/images/<user_id>/`) for persistent access by LibreChat.
- This approach is **not compatible** with the main branch or with generic MCP clients, as it relies on LibreChat's static file serving and user/session context.
- If you use the main branch, images are **not saved locally**; instead, the server returns a direct link to the image hosted on together.ai. These links are ephemeral and the images are deleted by together.ai after a short period (often less than 1 minute).

**Choose this branch only if you are deploying with LibreChat and require persistent, user-accessible images. For all other use cases, use the main branch.**
